...
geneva_1 = folium.Map(location=[lat, lng], zoom_start=zoom_start[rest_district])
geneva_2 = folium.Map(location=[lat, lng], zoom_start=zoom_start[rest_district])
geneva_3 = folium.Map(location=[lat, lng], zoom_start=zoom_start[rest_district])
geneva_4 = folium.Map(location=[lat, lng], zoom_start=zoom_start[rest_district])
geneva_5 = folium.Map(location=[lat, lng], zoom_start=zoom_start[rest_district])
...
if len(df)!=0:
    for i,row in df.iterrows():
        folium.Marker(
            ...
            popup=folium.Popup(f"Restaurant name: <b>{row['name']}</b>", max_width='300')
            ).add_to(marker_cluster)

# folium.LayerControl().add_to(geneva_1) # this gives a weird layer selection, so removed for the moment - Burak

## Map 02 - Price Levels
group0 = folium.FeatureGroup(name=f'<span style="color: {c["red"]}">$$$$</span>')
group1 = folium.FeatureGroup(name=f'<span style="color: {c["orange"]}">$$$</span>')
group2 = folium.FeatureGroup(name=f'<span style="color: {c["lgreen"]}">$$</span>')

for i,row in df.iterrows():
    lat = row['geometry.location.lat']
    lng = row['geometry.location.lng']
    rest_name = row['name']
    rest_prix = row['price_level_combined']

    if rest_prix < 3:
        folium.CircleMarker(location=[lat, lng],
                            radius=4, color=c['lgreen'], fill=True,
                            popup=folium.Popup(f"<b>{rest_name}</b><br>Price Level:{rest_prix}", max_width='300')).add_to(group2)

    elif rest_prix < 4 and rest_prix >= 3:
        folium.CircleMarker(location=[lat, lng],
                            radius=4, color=c['orange'], fill=True,
                            popup=folium.Popup(f"<b>{rest_name}</b><br>Price Level:{rest_prix}", max_width='300')).add_to(group1)

    elif rest_prix >= 4:
        folium.CircleMarker(location=[lat, lng],
                            radius=4, color=c['red'], fill=True,
                            popup=folium.Popup(f"<b>{rest_name}</b><br>Price Level:{rest_prix}", max_width='300')).add_to(group0)
...
group0 = folium.FeatureGroup(name=f'<span style="color: {c["red"]}">below 3</span>')
group1 = folium.FeatureGroup(name=f'<span style="color: {c["orange"]}">between 3 and 4</span>')
group2 = folium.FeatureGroup(name=f'<span style="color: {c["lgreen"]}">between 4 and 4.5</span>')
group3 = folium.FeatureGroup(name=f'<span style="color: {c["green"]}">above 4.5</span>')
...
for i,row in df.iterrows():
    ...
    lng = row['geometry.location.lng']
    rest_name = row['name']
    rest_rate = row['combined_rating']

    if rest_rate < 3.0:
        folium.CircleMarker(location=[lat, lng],radius=4, color=c['red'], fill=True,
                            popup=folium.Popup(f"<b>{rest_name}</b><br>Rate:{rest_rate}", max_width='300')).add_to(group0)
    elif rest_rate>=3.0 and rest_rate<4.0:
        folium.CircleMarker(location=[lat, lng], radius=4, color=c['orange'], fill=True,
                            popup=folium.Popup(f"<b>{rest_name}</b><br>Rate:{rest_rate}", max_width='300')).add_to(group1)
    elif rest_rate>=4.0 and rest_rate<4.5:
        folium.CircleMarker(location=[lat, lng], radius=4, color=c['lgreen'], fill=True,
                            popup=folium.Popup(f"<b>{rest_name}</b><br>Rate:{rest_rate}", max_width='300')).add_to(group2)
    elif rest_rate>=4.5:
        folium.CircleMarker(location=[lat, lng], radius=4, color=c['green'], fill=True,
                            popup=folium.Popup(f"<b>{rest_name}</b><br>Rate:{rest_rate}", max_width='300')).add_to(group3)
...
group0 = folium.FeatureGroup(name=f'<span style="color: {c["red"]}">less then 50 ratings</span>')
group1 = folium.FeatureGroup(name=f'<span style="color: {c["orange"]}">between 50 and 149 ratings</span>')
group2 = folium.FeatureGroup(name=f'<span style="color: {c["lgreen"]}">between 150 and 199 ratings</span>')
group3 = folium.FeatureGroup(name=f'<span style="color: {c["green"]}">more then 200 rantings ratings</span>')

for i,row in df.iterrows():
    lat = row['geometry.location.lat']
    lng = row['geometry.location.lng']
    rest_name = row['name']
    tot = row['user_ratings_total']

    if tot<50:
        folium.CircleMarker(location=[lat, lng], radius=4, color=c['red'], fill=True,
                            popup=folium.Popup(f"<b>{rest_name}</b><br>Total Ratings: {tot}", max_width='300')).add_to(group0)
    elif tot>=50 and tot<150:
        folium.CircleMarker(location=[lat, lng], radius=4, color=c['orange'], fill=True,
                            popup=folium.Popup(f"<b>{rest_name}</b><br>Total Ratings: {tot}", max_width='300')).add_to(group1)
    elif tot>=150 and tot<200:
        folium.CircleMarker(location=[lat, lng], radius=4, color=c['lgreen'], fill=True,
                            popup=folium.Popup(f"<b>{rest_name}</b><br>Total Ratings: {tot}", max_width='300')).add_to(group2)
    elif tot>=200:
        folium.CircleMarker(location=[lat, lng], radius=4, color=c['green'], fill=True,
                            popup=folium.Popup(f"<b>{rest_name}</b><br>Total Ratings: {tot}", max_width='300')).add_to(group3)
