...
for i in range(len(best_locations)):
    dfnew = data[data['district_cluster']==best_locations['district_cluster'][i]]
    list_of_points = np.array(dfnew[['geometry.location.lat', 'geometry.location.lng']])

    create_convexhull_polygon(geneva,
                              list_of_points,
                              layer_name = best_locations['district'][i],
                              line_color = 'green',
                              fill_color = 'lighblue',
                              weight=5, text = best_locations['district'][i])
    folium.CircleMarker(
...
for i in range(len(worst_locations)):
    dfnew = data[data['district_cluster']==worst_locations['district_cluster'][i]]
    list_of_points = np.array(dfnew[['geometry.location.lat', 'geometry.location.lng']])

    create_convexhull_polygon(geneva,
                            list_of_points,
                            layer_name = worst_locations['district'][i],
                            line_color = 'red',
                            fill_color = 'lighblue',
                            weight=5, text = worst_locations['district'][i])
